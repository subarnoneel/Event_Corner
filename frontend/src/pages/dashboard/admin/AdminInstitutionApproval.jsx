...
const AdminInstitutionApproval = () => {
  ...
  const fetchPendingInstitutions = async () => {
    try {
      setLoading(true);
      const response = await axios.get(API_ENDPOINTS.ADMIN_PENDING_INSTITUTIONS, {
  ...
  if (loading && pendingInstitutions.length === 0) {
    return (
...
export default AdminInstitutionApproval;