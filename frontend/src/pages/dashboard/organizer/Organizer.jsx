      ...
      >
        ...
        <div className="p-6 border-b border-gray-200 relative z-10">
          <div className="flex items-center justify-between">
            {sidebarOpen && (
              <div>
                <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Organizer Panel</h1>
                <p className="text-gray-500 text-xs mt-1">Organizer Dashboard</p>
              </div>
            )}
            <button
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-purple-300 rounded-r-full" />
                )}
              </Link>
            );
          })}
          ...
          {sidebarOpen && user && (
            <div className="mb-4 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
              ...
              {userData?.institution && (
                <p className="text-xs text-purple-600 mt-1 truncate">{userData.institution}</p>
              )}
            </div>
          )}
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 rounded-xl transition-all duration-200 text-white font-medium shadow-lg hover:shadow-xl hover:scale-105"
          >
            <FiLogOut size={18} />
