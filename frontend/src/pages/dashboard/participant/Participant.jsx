...
const Participant = () => {
  ...
  const getCurrentMenuItem = () => {
    ...
    if (currentPath.includes('/transactions')) return menuItems[4];
    return menuItems[0];
  };
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-gray-100">
      ...
      >
        {/* Decorative element */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-teal-400 to-teal-600 opacity-5 rounded-full -mr-16 -mt-16" />
        ...
        <div className="p-6 border-b border-gray-200 relative z-10">
          <div className="flex items-center justify-between">
            {sidebarOpen && (
              <div>
                <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Participant Panel</h1>
                <p className="text-gray-500 text-xs mt-1">Participant Dashboard</p>
              </div>
            )}
            <button
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-teal-300 rounded-r-full" />
                )}
              </Link>
            );
          })}
          ...
          {sidebarOpen && user && (
            <div className="mb-4 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
              ...
              {userData?.institution && (
                <p className="text-xs text-teal-600 mt-1 truncate">{userData.institution}</p>
              )}
            </div>
          )}
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 rounded-xl transition-all duration-200 text-white font-medium shadow-lg hover:shadow-xl hover:scale-105"
          >
            <FiLogOut size={18} />
